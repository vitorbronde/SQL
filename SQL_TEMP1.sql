DROP TABLE IF exists getninjas_temp.estudo_conversao;
CREATE TABLE getninjas_temp.estudo_conversao AS (
with visu as (
SELECT distinct 
    vo.profile_id ,
    vo.request_id,
...
FROM getninjas_dispatch.visualized_offers vo
    inner join getninjas_pivots.enriched_requests er on (vo.request_id = er.request_id)
    inner join getninjas_intermediary.profiles_address_root AS par ON (par.profile_id = vo.profile_id)
    join getninjas_pivots.enriched_profiles AS ep on (vo.profile_id = ep.profile_id)
...
request_id , 
count(*) as views  
from getninjas_dispatch.visualized_offers 
where date between '2022-01-01' and '2022-04-10'
group by 1,2
),
...
from getninjas_dispatch.enriched_leads el 
